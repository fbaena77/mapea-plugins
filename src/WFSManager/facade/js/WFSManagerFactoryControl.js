goog.provide('P.control.WFSManagerFactoryControl');

/**
 * @classdesc
 * Main constructor of the class. Creates a WFSManagerFactoryControl
 * control
 *
 * @constructor
 * @extends {M.Control}
 * @api stable
 */
M.control.WFSManagerFactoryControl = (function (base) {
	/**
	 * Base control
	 * @private
	 * @type {Object}
 ...
 * @api stable
 */
M.control.WFSManagerFactoryControl.prototype.initLayersFromNode = function (node) {
 ...
 * @api stable
 */
M.control.WFSManagerFactoryControl.prototype.createLayersFromNode = function (node) {
	if(node.type === "layer"){
 ...
 * @api stable
 */
M.control.WFSManagerFactoryControl.prototype.createLayerFromNode = function (node) {
	...
	var layerOptions = {
			format: node.format,
			minResolution: node.minResolution ? node.minResolution : this.minResolutionDefault,	
			maxResolution: node.maxResolution ? node.maxResolution : this.maxResolutionDefault,
	...
	if(node.layerInfo.url && node.layerInfo.url.indexOf("_vector_file:")>-1){
		...
		for(let i=0;i<this.base_.tocObjectForLoad_.vectors.length;i++){
			if(this.base_.tocObjectForLoad_.vectors[i].file===fileRef){
				...
				this.base_.getImpl().createVectorLayerFromGeoJson(routeJSON, node.layerInfo.name, node.active);
			}
		}
	}
